...
function r1_cmp($a, $b)
{
    $a_s = strtotime($a->Data_qest);//substr($a->Data_qest, 6, 4) . substr($a->Data_qest, 3, 2) . substr($a->Data_qest, 0, 2);
    $b_s = strtotime($b->Data_qest);;//substr($b->Data_qest, 6, 4) . substr($b->Data_qest, 3, 2) . substr($b->Data_qest, 0, 2);
	if ($a_s == $b_s) {
		return 0;
	}
	return ($a_s < $b_s) ? 1 : -1;
	...
	foreach ( $attachments as $id => $attachment ) {
		...
		$output .= "
			...
			$output .= "
				...
				" . wptexturize($attachment->Time_qest) . "
				</{$captiontag}>";
		
		
			$output .= "
				<{$captiontag} class='gallery-description' id='$selector-$id'>
				" . wptexturize($attachment->Data_qest) . "
				</{$captiontag}>";
